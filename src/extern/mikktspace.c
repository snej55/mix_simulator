...
static void GenerateSharedVerticesIndexList(int piTriList_in_and_out[], const SMikkTSpaceContext* pContext,
...
static void DegenEpilogue(STSpace psTspace[], STriInfo pTriInfos[], int piTriListIn[],
                          const SMikkTSpaceContext* pContext, const int iNrTrianglesIn, const int iTotTris);
    ...
    for (f = 0; f < iNrFaces; f++)
    {
        ...
        for (i = 0; i < verts; i++)
        {
            ...
            if (pContext->m_pInterface->m_setTSpaceBasic != NULL)
                pContext->m_pInterface->m_setTSpaceBasic(pContext, tang, pTSpace->bOrient == TTRUE ? 1.0f : (-1.0f), f,
                                                         i);
...
static void MergeVertsFast(int piTriList_in_and_out[], STmpVert pTmpVert[], const SMikkTSpaceContext* pContext,
                           const int iL_in, const int iR_in);
static void MergeVertsSlow(int piTriList_in_and_out[], const SMikkTSpaceContext* pContext, const int pTable[],
                           const int iEntries);
static void GenerateSharedVerticesIndexListSlow(int piTriList_in_and_out[], const SMikkTSpaceContext* pContext,
                                                const int iNrTrianglesIn);
        ...
        if (iFO_a == iFO_b) // this is a quad
        {
            ...
            if ((bIsDeg_a || bIsDeg_b) == TFALSE)
            {
                ...
                if (bOrientA != bOrientB)
                {
                    ...
                    else if (CalcTexArea(pContext, &piTriListIn[t * 3 + 0]) >= CalcTexArea(
                        pContext, &piTriListIn[(t + 1) * 3 + 0]))
    ...
    for (i = 0; i < iEntries; i++)
    {
        ...
        if (bUnassigned_A)
        {
            ...
            while (j < iEntries && i0 == pEdges[j].i0 && i1 == pEdges[j].i1 && bNotFound)
            {
                ...
                t = pEdges[j].f;
                // flip i0_B and i1_B
                GetEdge(&i1_B, &i0_B, &edgenum_B, &piTriListIn[t * 3], pEdges[j].i0, pEdges[j].i1);
